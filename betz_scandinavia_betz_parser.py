...
LEAGUE_NAME_MAP = {
    "швеция. аллсвенскан":    ("SWE", "Швеция. Аллсвенскан"),
    "норвегия. элитесериен":  ("NOR", "Норвегия. Элитесериен"),
    "дания. суперлига":       ("DNK", "Дания. Суперлига"),
    "финляндия. вейккаусл":   ("FIN", "Финляндия. Вейккаусліга"),
    "ирландия. премьер":      ("IRL", "Ирландия. Премьер-лига"),
}

def get_league_key(league_name: str):
    ln = league_name.lower()
    for key, val in LEAGUE_NAME_MAP.items():
        if key in ln:
            return val
    return None
...
def parse_day(html: str, date_str: str, session: requests.Session, delay: float) -> list:
    ...
    for league_div in cc_divs:
        ...
        mapped = get_league_key(league_name)
        if not mapped:
            continue
...
def save_to_results_raw(conn, rows) -> int:
    """Write finished matches to results_raw for live settlement.

    Only rows with a known score (home_score/away_score not None) are written.
    Uses INSERT OR IGNORE with a unique index guard on (league, match_date, home_team, away_team).
    ...
    for r in rows:
        if r.get("home_score") is None or r.get("away_score") is None:
            continue
        # league stored in results_raw is the full human-readable name (e.g. "Швеция. Аллсвенскан")
        league_raw = r.get("league_name") or r.get("league", "")
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, 1, 'betz_scandinavia', datetime('now'))
        """, (league_raw, r["match_date"], r["home_team"], r["away_team"],
...
def save_rows(conn, rows):
    ...
    for r in rows:
        ...
        """, (r["league"], r["season"], r["home_team"], r["away_team"], r["match_date"])).fetchone()
...
def main():
    ...
    ap.add_argument("--days", type=int, default=None,
                    help="Fetch last N days (overrides --date-from for settlement use-case)")
    ap.add_argument("--db", default=DB_PATH)
    ap.add_argument("--delay", type=float, default=0.5)
    args = ap.parse_args()

    # --days N shortcut: last N days ending at --date-to
    if args.days is not None:
        dt_end = datetime.strptime(args.date_to, "%Y-%m-%d")
        date_from = (dt_end - timedelta(days=args.days - 1)).strftime("%Y-%m-%d")
    else:
        date_from = args.date_from or "2021-04-01"
    ...
    while dt <= dt_end:
        ...
        if html:
            ...
            if rows:
                ...
                raw_ins = save_to_results_raw(conn, rows)
                total_ins += ins
                total_upd += upd
                total_raw += raw_ins
                log.info(f"{date_str_req}: +{ins} новых, ~{upd} обновлено | results_raw +{raw_ins}")

        dt += timedelta(days=1)
        time.sleep(args.delay)

    conn.close()
    log.info(f"Готово! Всего: +{total_ins} новых, ~{total_upd} обновлено | results_raw +{total_raw}")
