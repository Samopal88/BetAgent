...
# Football-specific exotic patterns to filter out
...
def ensure_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS backtest_football_matches_betz (
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_bfbm_date ON backtest_football_matches_betz(match_date)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_bfbm_league_season ON backtest_football_matches_betz(league, season)"
    )
    ensure_column(conn, "backtest_football_matches_betz", "odds_btts_yes", "REAL")
    ensure_column(conn, "backtest_football_matches_betz", "odds_btts_no", "REAL")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_bfbm_teams ON backtest_football_matches_betz(home_team, away_team)"
    )
    conn.commit()
...
def is_special_market_match(home_team: str, away_team: str) -> bool:
    bad_prefixes = ("УГЛ ", "ЖК ", "ФОЛЫ ", "УДАРЫ ", "ОФСАЙДЫ ")
    return home_team.startswith(bad_prefixes) or away_team.startswith(bad_prefixes)


def parse_score_and_periods(result_text: str) -> tuple[Optional[int], Optional[int], str, Optional[str], Optional[str]]:
    ...
    if "бул" in lowered or "пен" in lowered:
        result_type = "penalties"
    elif "от" in lowered or "д.в" in lowered:
        result_type = "ot"
    elif score_match:
        result_type = "regular"
    return home_score, away_score, result_text, periods, result_type
...
def extract_named_odd(text: str, label: str) -> Optional[float]:
    m = re.search(rf"{re.escape(label)}\s*-\s*(\d+(?:\.\d+)?)", text, re.IGNORECASE)
    if not m:
        return None
    try:
...
def parse_detail_markets(detail_html: str) -> dict[str, Optional[float]]:
    ...
    out["odds_btts_yes"] = extract_named_odd(text, "Обе забьют Да")
    out["odds_btts_no"] = extract_named_odd(text, "Обе забьют Нет")
...
def parse_page(html: str, requested_dt: date, page_num: int, page_url: str) -> tuple[list[MatchRow], bool]:
    ...
    for li in root.find_all("li", recursive=False):
        ...
        if is_exotic_match(current_league, home_team, away_team):
            continue
        if is_special_market_match(home_team, away_team):
            continue
        ...
        home_score, away_score, result_raw, periods_raw, result_type = parse_score_and_periods(result_text)
...
def upsert_rows(conn: sqlite3.Connection, rows: list[MatchRow]) -> tuple[int, int, int]:
    ...
    sql = """
    INSERT INTO backtest_football_matches_betz (
    ...
    for row in rows:
        existed = conn.execute(
            "SELECT 1 FROM backtest_football_matches_betz WHERE source=? AND source_match_id=?",
...
def process_date(
    ...
    while True:
        if test_html:
            ...
            logging.info(
                "Дата %s | страница %s | %s", dt.isoformat(), page_num + 1, url
            )
            html = downloader.fetch_html(url)

        rows, has_next = parse_page(html, dt, page_num, url)
        logging.info(
            "Дата %s | страница %s | найдено матчей: %s",
            dt.isoformat(),
            page_num + 1,
            len(rows),
        )
        ...
        for row in rows:
            try:
                if row.detail_url and not test_html:
                    detail_html = downloader.fetch_html(row.detail_url)
                    extra = parse_detail_markets(detail_html)
                    row.odds_btts_yes = extra.get("odds_btts_yes")
                    row.odds_btts_no = extra.get("odds_btts_no")
            except Exception as exc:
                logging.warning("detail parse failed for %s: %s", row.detail_url, exc)
            enriched_rows.append(row)

        inserted, updated, skipped = upsert_rows(conn, enriched_rows)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Загрузка исторических футбольных матчей с betz.su"
    )
    ...
    parser.add_argument(
        "--delay", type=float, default=0.8, help="Пауза между запросами страниц"
    )
    parser.add_argument(
        "--max-pages", type=int, default=20, help="Макс. число страниц на дату"
    )
