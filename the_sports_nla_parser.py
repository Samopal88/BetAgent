...
Парсит страницу TheSports по НХЛ:
- standings
- recent results
- upcoming fixtures
...
Запуск:
  ...
  python the_sports_nhl_parser.py --html nhl_page.html
"""
...
DEFAULT_URL = "https://www.the-sports.org/ice-hockey-2025-2026-switzerland-nationalliga-a-epr137212.html"
...
LEAGUE = "nla"
LEAGUE_NAME = "Хоккей. Швейцария. National League."

# Маппинг названий команд из Фонбета → нормализованные русские названия
FONBET_TO_RU = {
...
def parse_standings(soup: BeautifulSoup) -> List[Tuple]:
    # NLA — standings в первой таблице с Pts/MP
    tables = soup.find_all("table")
    table = None
    for t in tables:
        if "Pts" in t.get_text() and "MP" in t.get_text():
            table = t
            break
    if not table:
...
def parse_results_and_fixtures(soup: BeautifulSoup, debug_scores: bool = False) -> List[Tuple]:
    ...
    for tr in content.find_all("tr"):
        ...
        m = re.search(r"(\d+)\s*[-–:]\s*(\d+)(?:\s*(ot|OT|so|SO|\(OT\)|\(SO\)))?(?:\s*\(.*\))?", score_text)
        if m:
            home_score = int(m.group(1))
            away_score = int(m.group(2))
            is_overtime = 1 if m.group(3) else 0
            is_finished = 1
        else:
            home_score = None
            away_score = None
...
def save_results_raw(conn: sqlite3.Connection, rows: List[Tuple]):
    ...
    payload = [
        (LEAGUE, r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], DEFAULT_URL, now)
        for r in rows
    ]
    cur.executemany("""
        INSERT OR REPLACE INTO results_raw (
            league, match_date, kickoff, home_team, away_team,
            home_score, away_score, is_finished, is_overtime, source, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, payload)
    conn.commit()
...
def upsert_matches(conn: sqlite3.Connection, rows: List[Tuple]):
    ...
    for match_date, kickoff, home_team, away_team, home_score, away_score, is_finished, is_overtime in rows:
        if not match_date:
            continue
